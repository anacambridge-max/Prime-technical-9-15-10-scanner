...
export async function getNifty500Symbols(): Promise<Instrument[]> {
  ...
  const [niftyRes, instrumentsRes] = await Promise.all([
    fetch('https://www.niftyindices.com/IndexConstituent/ind_nifty500list.csv', {
      headers: {
        'User-Agent': 'Mozilla/5.0 PrimeTechnicalScanner',
  ...
  const nseSymbols = new Set(
    csv.split(/\r?\n/).slice(1)
      .map((line) => line.split(',')[2]?.replaceAll('"', '').trim().toUpperCase())
      .filter(Boolean),
  );

  const raw = await instrumentsRes.json() as unknown;
...
export async function getFiveMinuteCandles(instrumentKey: string, fromDate: string, toDate: string): Promise<Candle[]> {
  ...
  return candles.filter(Array.isArray).map((c) => ({
    timestamp: String(c[0]), open: Number(c[1]), high: Number(c[2]), low: Number(c[3]), close: Number(c[4]), volume: Number(c[5]),
